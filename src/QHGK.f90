  ...
  INTEGER  :: natms, ndim, i, j, k, iargc, idx,ntype, jj
  ...
  LOGICAL :: lsijx = .false., ltubez = .false., lpr =.false.
  LOGICAL :: lsijy = .false.
  LOGICAL :: lsijz = .false.
  ...
  if(iargc()<1) stop "Usage: AF.x <#atoms> <#nspecies> [-b] [-tube <diameter>]"
  ...
  IF (iargc()>2) then 
    do i = 3, iargc()
      ...
      if (INDEX(buffer,'-tube').NE.0) then
        ltubez = .true.
        CALL GETARG(i+1,buffer)
        READ(buffer,*) radius
        radius = radius /2.d0
...
! CALL fconst(ndim)
  ...
  if (lsijx .or. lsijy .or. lsijz .or. ltubez) then
    write(*,*) "Build heatflux matrix elements"
    if(ltubez) CALL buildSij(ndim,dynmat,3)
  ...
  if(ltubez) then 
    volume = cell(9)*pi*radius**2
    CALL diffuz(ndim,eigenvalues,3)
  else
    !CALL diffusivity(ndim, eigenvalues, linewidth)
...
!      temp = DBLE(i)*dtemp
     ...
     do j = 4+istart, ndim
       ...
       cvqm = cvqm + cvx
       kappaCL = kappaCL + kboltz*diff(1,j)/volume
       kappaQM = kappaQM + kboltz*cvx*diff(1,j)/volume
       write(103,'(4g16.6)') omega(j)/cnv, kappaQM, kappaCL, kboltz*cvx*diff(1,j)/volume
     enddo 
...
!~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
     SUBROUTINE diffuz(ndim, omega, ix)
     IMPLICIT NONE
     ...
     delta = 0.1 ! Hard-coded delta for 1D materials
     if (ltubez) then
        istart = 1
     else
        istart = 0
     ...
     do i = 4+istart, ndim
       ...
       enddo
       diff(i) = pi*diff(i)/omega(i)**2/100.d0 ! convert to mm^2/s
     enddo

     OPEN (unit=88,file='diffusivity')
     do i = 4+istart, ndim
        write(88,'(6g14.6)') omega(i)/2.d0/pi, diff(i)
     enddo
     close(88)
     deallocate(ework)
